...
with $F$ the contact force pressing the clast against the bed, $\eta$ the ice viscosity, $R$ the clast radius, and $v_n$ the ice velocity component carrying the clast toward the bed. The force is the viscous drag of ice creeping past the clast, and it grows in direct proportion to each of the three: stiffer ice, a larger clast, or a faster squeeze toward the bed all press the tool down harder. The numbers are startling. For $\eta \approx 3\times10^{12}$ Pa s, a half-meter boulder, and a modest meter per year of convergence, the contact force is about $10^6$ N, the weight of a hundred tons of rock pressing down a boulder that itself weighs less than one. This is why striations appear on vertical and even overhanging surfaces, where gravity could never press a tool against the rock. The abrasion rate then scales as
...
with $\dot A$ the abrasion rate expressed as a lowering of the bed, $F_c$ the contact force on a clast, $v_p$ the particle speed, $C$ the concentration of tools on the bed, and $\alpha$ a constant absorbing rock hardness {cite}`hallet1979`. The product $F_c\,v_p$ is the wear cut by a single tool, harder pressing and faster sliding each deepening the groove, and multiplying by $C$ sums the work of all the tools acting on a unit area at once. Since both $F_c$ and $v_p$ grow with the sliding speed, abrasion goes roughly as the square of sliding, an energy statement, since $\mu F_c v_p C$ is the frictional work spent per unit bed area.
