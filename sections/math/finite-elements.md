...
Each term is a rate of virtual work done against the test field $\mathbf{w}$. The first, the volume integral of $\nabla\mathbf{w}:\boldsymbol{\sigma}$, is the internal work of the stress acting through the gradient of the virtual velocity, with $\nabla\mathbf{w}:\boldsymbol{\sigma}$ the double contraction that pairs the two tensors into a scalar. The second, over the boundary $\partial\Omega$ with outward normal $\mathbf{n}$, is the work of the surface traction $\boldsymbol{\sigma}\mathbf{n}$. The third is the work of gravity, the ice density $\rho_i$ times the gravitational acceleration $\mathbf{g}$, throughout the volume. This is the weak, or variational, form. Two things have been gained. The solution now needs only one derivative rather than two, so the piecewise-polynomial approximations introduced below are admissible. And the boundary integral has made the natural boundary conditions explicit: the term $\boldsymbol{\sigma}\mathbf{n}$ is the traction on the ice surface, so a stress-free upper surface, the water pressure at a calving front, and the ocean pressure beneath a shelf all enter here, simply by substituting the known traction. The conditions that prescribe the velocity itself, such as no slip on a frozen bed or a given inflow, are imposed differently, as described below.
