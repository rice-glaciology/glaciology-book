...
The bond energies and shell capacities quoted in the previous chapter are quantum-mechanical in origin, and classical mechanics offers no substitute; a classical electron orbiting a nucleus would radiate away its energy and fall into the nucleus in about $10^{-11}$ s, and a classical lattice would show the wrong heat capacity at every temperature, for the reasons previewed in the {doc}`preface`. This chapter develops the quantum-mechanical results the book requires, the origin of discrete energy levels, the hydrogen atom and the shell structure that follows from it, the harmonic oscillator and rigid rotor that describe molecular vibration and rotation, and the comparison of level spacing with $k_B T$ that determines when classical reasoning is adequate. The standard results are quoted without formal solution of the wave equation, but each is derived or verified by an estimate simple enough to repeat, and each is applied directly to the water molecule.
