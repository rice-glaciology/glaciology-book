...
## Defects and why disorder matters

The proton arrangement is not frozen for all time. Protons can move between allowed configurations through point defects in the lattice. Ionic defects appear when a proton hops across a bond to leave an $\mathrm{H_3O^+}$ and an $\mathrm{OH^-}$ pair, and orientational, or Bjerrum, defects appear when a molecule rotates to leave a bond with two protons or none. The slow migration of these defects allows the protons to rearrange in response to an applied electric field, which is the origin of the dielectric relaxation and the weak electrical conductivity of ice. These are exactly the processes that determine how ice responds to the oscillating field of a radar wave, so the proton disorder of the crystal connects back to the geophysical methods used to study it.

Ice can take many other crystal structures at high pressure, more than a dozen distinct phases, but within glaciers and ice sheets the pressures are far too low to reach them, and the ice is ice Ih throughout. From the bent water molecule, then, follow the open tetrahedral crystal that floats, the symmetry axis that shapes how ice flows and how fabric forms, and the proton disorder that governs how ice responds to an electric field.
