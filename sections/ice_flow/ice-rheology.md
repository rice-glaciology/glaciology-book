...
where $\dot\varepsilon$ is the total strain rate and the four contributions are the rates of the separate deformation mechanisms, $\dot\varepsilon_{\mathrm{diff}}$ for diffusional flow, $\dot\varepsilon_{\mathrm{disl}}$ for dislocation creep, and $\dot\varepsilon_{\mathrm{basal}}$ and $\dot\varepsilon_{\mathrm{gbs}}$ for basal slip and grain-boundary sliding. The two isolated terms are mechanisms that proceed independently, so their rates add at a common stress. The bracketed combination of inverses is the pair of basal slip and grain-boundary sliding, which must operate together because each must wait on the other, so whichever is slower sets the pace of the pair. Each term is a power law $\dot\varepsilon_{(\cdot)} = A_{(\cdot)}\tau^{n_{(\cdot)}}$ of its own exponent, diffusional flow at $n=1$, grain-boundary sliding near $n=1.8$, basal slip near $n=2.4$, and dislocation creep near $n=4$. At higher stresses some recent analyses favor $n$ closer to 4. The honest position is that a single power law is an approximation to a composite of mechanisms, each dominant in a different range of stress, temperature, and grain size, and that $n=3$ is a serviceable average across the conditions of most glacier flow rather than a fundamental constant. Because the strain rate depends so steeply on stress, the nonlinearity is responsible for much of the character of ice flow, including the tendency of fast flow to concentrate into narrow streams: a region carrying twice the deviatoric stress of its surroundings deforms about eight times faster.
...
in which $A_0$ is a temperature-independent prefactor, $T'$ is the temperature measured relative to the pressure-melting point, the homologous temperature, $R$ is the gas constant, and $Q$ is the activation energy for creep. The exponential is the fraction of thermal events energetic enough to clear the barrier $Q$, so the rate factor climbs steeply as the ice warms, while $A_0$ sets the overall scale that the exponential modulates. Using the homologous temperature matters at depth, where the high pressure lowers the melting point by a few tenths of a degree per hundred meters and makes the deep ice effectively warmer than its absolute temperature suggests.
